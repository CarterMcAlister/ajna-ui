...
export const ChakraReactSelect = ({
  ...
  const chakraTheme = useTheme()
  const [lightBg, darkBg] = useToken('colors', ['white', 'gray.600'])
  const bgColor = useColorModeValue(lightBg, darkBg)
  const textColor = useColorModeValue('black', lightBg)
  ...
  const backgroundColor = useColorModeValue(
    ...
    chakraTheme.colors.transparent
  )
  ...
  const select = cloneElement(children, {
    ...
    styles: {
      ...
      input: (base) => ({ ...base, lineHeight: 1, color: textColor }),
      control: (base) => ({ ...base, background: bgColor }),
      menu: (base) => ({
        ...base,
        boxShadow: 'none',
      }),
      option: (base, state) => ({
        ...base,
        color: !state.isFocused && colorMode === 'dark' ? 'white' : 'black',
